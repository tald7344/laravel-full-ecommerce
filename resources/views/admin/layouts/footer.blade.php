  <footer class="main-footer">
    <strong>Copyright &copy; {{ date('Y', time()) }} <a href="#">Talal Danoun</a>.</strong>
    {{ trans('admin.all-right-reserved') }}
...
<script>
  ...
  // Get Chart Data and month days
  var chartData = [];
  var chartDataLabels = [];
  @for($i = 1; $i <= cal_days_in_month(CAL_GREGORIAN, date('m', time()), date('Y', time())); $i++)
      chartDataLabels.push('{{ $i }}');
      chartData.push('{{ generate_daily_visits_count(date('Y', time()), date('m', time()), $i) }}');
  @endfor

  var graphChartData = [];
  @for($i = 1; $i <= 12; $i++)
    graphChartData.push('{{ generate_monthly_orders_count(date('Y', time()), $i) }}');
  @endfor
</script>
<!--DataTable-->
