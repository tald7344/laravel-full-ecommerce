...
class StateDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('checkbox', 'admin.states.actions.checkbox')
            ->addColumn('edit', 'admin.states.actions.edit')
            ->addColumn('delete', 'admin.states.actions.delete')
    ...
    public function query(State $model)
    {
        ...
        return $model->newQuery()->with('country_id')->with('city_id')->select('states.*');
        // return City::query()->with('country_id')->select('states.*');
    }
    ...
    {
        return $this->builder()
        ->setTableId('statesdatatable-table')
        ...
        ->parameters([
            ...
            'buttons' => [
                [
                    'text' => '<i class="fa fa-plus mr-1"></i> ' . trans('admin.new_state'),
    ...
    protected function getColumns()
    {
        return [
            ...
            ->printable(false),
            Column::make('id'),
            Column::make('states_name_ar')->title(trans('admin.states_name_ar')),
            Column::make('states_name_en')->title(trans('admin.states_name_en')),
            Column::computed('country_id')
              ...
              ->searchable(false)
              ->printable(false),
            Column::computed('city_id')
              ->title(trans('admin.city_id'))
              ->data('city_id.cities_name_' . lang())
              ->exportable(false)
              ->orderable(false)
              ->searchable(false)
              ->printable(false),
