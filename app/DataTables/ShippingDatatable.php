...
class ShippingDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('checkbox', 'admin.shippings.actions.checkbox')
            ->addColumn('icon', 'admin.shippings.actions.icon')
            ->addColumn('edit', 'admin.shippings.actions.edit')
            ->addColumn('delete', 'admin.shippings.actions.delete')
    ...
    {
        return $model->newQuery()->with('user_id')->select('shippings.*');
    }
    ...
    {
        return $this->builder()
        ->setTableId('shippingsdatatable-table')
        ...
        ->parameters([
            ...
            'buttons' => [
                [
                    'text' => '<i class="fa fa-plus mr-1"></i> ' . trans('admin.new_shipping'),
    ...
    protected function getColumns()
    {
        return [
            ...
            ->printable(false),
            Column::make('id'),
            Column::make('shippings_name_ar')->title(trans('admin.shippings_name_ar')),
            Column::make('shippings_name_en')->title(trans('admin.shippings_name_en')),
            Column::make('user_id')
            ->title(trans('admin.owner_id'))
            ->data('user_id.name'),
            Column::computed('icon')
            ->title(trans('admin.shipping_icon'))
            ->exportable(false)
            ->orderable(false)
            ->searchable(false)
            ->printable(false),
