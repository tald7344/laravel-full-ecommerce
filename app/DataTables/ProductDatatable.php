...
class ProductDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('checkbox', 'admin.products.actions.checkbox')
            ->addColumn('is_hot', 'admin.products.actions.hot')
            ->addColumn('edit', 'admin.products.actions.edit')
            ->addColumn('delete', 'admin.products.actions.delete')
            ->addColumn('photo', 'admin.products.actions.photo')
            ->addColumn('updated_at', 'admin.admins.actions.updated_at')
            ->addColumn('created_at', 'admin.admins.actions.created_at')
            ->rawColumns([
                'checkbox', 'edit', 'delete', 'photo', 'is_hot', 'updated_at', 'created_at'
            ]);
    }
    ...
    public function html()
    {
        ...
        ->parameters([
            ...
            'buttons' => [
                [
                    'text' => '<i class="fa fa-plus mr-1"></i> ' . trans('admin.new_product'),
                    'className' => 'btn btn-info text-white',
                    'action' => 'function () {
                        window.location.href = "' . URL::current() . '/create";
                    }'
                ],
                ['extend' => 'print', 'className' => 'btn btn-primary text-white m-1', 'text' => '<i class="fa fa-print"></i>'],
                ['extend' => 'csv', 'className' => 'btn btn-info text-white m-1', 'text' => '<i class="fa fa-file mr-2"></i> ' . trans('admin.export_csv')],
                ['extend' => 'excel', 'className' => 'btn btn-success text-white m-1', 'text' => '<i class="fa fa-file mr-2"></i> ' . trans('admin.export_excel')],
                ['extend' => 'reload', 'className' => 'btn btn-outline-secondary m-1', 'text' => '<i class="fa fa-sync"></i>'],
                [
                    'text' => '<i class="fa fa-trash mr-1"></i> ' . trans('admin.delete_all'),
                    'className' => 'btn btn-danger text-white m-1 delete_all',
                ],
            ],
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::computed('photo')
              ...
              ->searchable(false)
              ->printable(false),
            Column::make('title_en')->title(trans('admin.title_en'))
              ->render("function(item) {
                  if (this.title_en) {
                    return '<span>' + String(this.title_en).substr(0, 40) + '...' + '</span>';
                  } else {
                    return '----'
                  }
              }"),
            Column::make('title_ar')->title(trans('admin.title_ar'))
              ->render("function() {
                  if (this.title_en) {
                    return '<span>' + String(this.title_ar).substr(0, 40) + '...' + '</span>';
                  } else {
                    return '----'
                  }
              }"),
